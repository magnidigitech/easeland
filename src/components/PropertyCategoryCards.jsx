...
import { LandPlot, Home, Building2, Building, Trees, Store, Key, ArrowRight } from 'lucide-react';
...
export default function PropertyCategoryCards({ onSelectCategory }) {
  ...
  const cmsCategories = siteConfig.categories || [];
  const categories = [
    {
      id: 'open-plots',
      name: 'Open Plots',
  ...
  return (
    <section className="py-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-10">
        ...
        <p className="text-sm text-gray-500 font-medium max-w-md mt-2 md:mt-0">
        ...
        {categories.map((cat) => {
          ...
          return (
            <div
              ...
              className="group bg-white rounded-2xl p-6 border border-brand-bordergray shadow-sm hover:shadow-xl hover:border-brand-yellow transition-all cursor-pointer flex flex-col justify-between transform hover:-translate-y-1"
            >
              <div>
                <div className="flex items-center justify-between mb-5">
                  <div className="w-12 h-12 rounded-xl bg-brand-offwhite group-hover:bg-brand-yellow text-brand-charcoal flex items-center justify-center transition-colors">
                    <IconComponent className="w-6 h-6 stroke-[2]" />
                  </div>
                  <span className="text-[11px] font-bold bg-brand-offwhite text-gray-600 group-hover:bg-brand-charcoal group-hover:text-brand-yellow px-2.5 py-1 rounded-full transition-colors">
                    {cat.badge}
                  </span>
                </div>

                <h3 className="text-lg font-bold text-brand-charcoal group-hover:text-black mb-1">
                  {cat.name}
                </h3>
              ...
              <div className="pt-4 border-t border-gray-100 flex items-center justify-between text-xs font-bold text-brand-charcoal group-hover:text-brand-yellow">
